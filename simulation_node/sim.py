...
class Rat:
    ...
    status = 0  # rat status, where 0 -> 1 -> 2 -> 3
    statusTime = 0  # keep status in each time
    ...
    def updateStatus(self, alpha = 1.0, beta = 0.5):
        prob = rutil.RNG.randFloat()
        if self.status == 0:
            exposedDensity = node.ratStatusCount[1] / node.ratCount
            infectiousDensity = node.ratStatusCount[2] / node.ratCount
            infectProb = alpha * infectiousDensity + beta * exposedDensity
            if prob < infectProb:
                self.status = 1
                self.node.ratStatusCount[0] -= 1
                self.node.ratStatusCount[1] += 1
        elif self.status == 1:
            exposedToInfectiousProb = 1.0 / (1.0 + math.pow(math.e, -(self.statusTime - 7)))
            if prob < exposedToInfectiousProb:
                self.node.ratStatusCount[1] -= 1 
                self.node.ratStatusCount[2] += 1
                self.status = 2
                self.statusTime = 0
            else:
                self.statusTime += 1
        elif self.status == 2:
            infectiousToRecoverProb = 1.0 / (1.0 + math.pow(math.e, -(self.statusTime - 7)))
            if prob < infectiousToRecoverProb:
                self.node.ratStatusCount[2] -= 1
                self.node.ratStatusCount[3] += 1
                self.status = 3
                self.statusTime = 0
            else:
                self.statusTime += 1
...
class Node:
    ...
    def addRat(self, r):
        self.ratCount += 1
        self.ratStatusCount[r.status] += 1
...
class Simulator:
    ...
    def loadRats(self, fname = "", seed = rutil.DEFAULTSEED):
        ...
        for line in f:
            ...
            if first:
                ...
                self.alpha, self.beta = map(int, line.split())
                second = False
            else:
        ...
        f.close()
        self.restart(ratPositions, seed, statusList)
    ...
    def restart(self, ratPositions = [], seed = rutil.DEFAULTSEED, statusList = []):
        self.rats = []
        for n in self.nodes:
            n.reset()
        self.time = 0
    ...
    def populationList(self):
        ...
        # return [nd.ratStatusCount[2] for nd in self.nodes]
    ...
    def driveOut(self, f = sys.stdout, display = True):
        f.write("STEP %d %d %d\n" % (self.width, self.height, self.ratCount()))
        if display:
    ...
    def simulate(self, stepCount = 1, update = UpdateMode.synchronous, displayInterval = 1):
        ...
        for step in xrange(stepCount):
            ...
            while ridx < len(self.rats):
                ...
                for i in xrange(bcount):
                    r = self.rats[i + ridx]
                    r.updateStatus(self.alpha, self.beta)
                ridx += bcount
            self.time += 1
            # Emit new state
