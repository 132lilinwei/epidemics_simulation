...
class VizSimulator(sim.Simulator):
    ...
    def show(self, period = 0.0, last = False):
        if self.formatter is None:
            self.formatter = viz.Formatter(self.width, self.height, self.ratCount(), viz = self.vizMode)
        else:
...
class DrivenSimulator(VizSimulator):
        ...
        for line in sys.stdin:
            ...
            if id == -1:
                ...
                if len(tokens) < 1 or tokens[0] != "STEP":
                    ...
                    return "ERROR"
                try:
                    width, height, self.nrats = map(int, tokens[1:])
                    ncount = width * height
    ...
    def simulate(self, stepCount = 1, update = sim.UpdateMode.synchronous, period = 0.0, displayInterval = 1):
        ...
        while True:
            ...
            self.time += 1
            realStepCount += 1
            print("cur step count: ", realStepCount)
