...
__device__ __inline__  double compute_single_infection_prob(int alpha, int beta, int rat_count, int exposed_rat_count, int infectious_rat_count) { // note here we only pass values of one node
    if (rat_count == 0) {
        ...
        double density_of_exposed = (double)exposed_rat_count / (double) rat_count;
        double density_of_infectious = (double)infectious_rat_count / (double) rat_count;
...
extern "C" void next_move_cuda(state_t *s, double batch_fraction){
    ...
    int hub_num_block_x = (nhub + HUB_BLOCK_SIZE - 1) / HUB_BLOCK_SIZE;
    dim3 hubGridDim(hub_num_block_x);
    next_move_hub_kernel<<<hubGridDim, hubBlockDim>>>(hub_device, nhub, rat_count_device, healthy_rat_count_device, exposed_rat_count_device, infectious_rat_count_device, weight_result_device, sum_weight_result_device, neighbor_device, neighbor_start_device, width, height, batch_fraction);
    // cudaThreadSynchronize();
    // printf("compute_weights: Overall hub: %.3f ms\t\t[%.3f GB/s]\n", 1000.f * (CycleTimer::currentSeconds()-startTime), toBW(totalBytes, (CycleTimer::currentSeconds()-startTime)));
    ...
    int num_block_y = (height+BLOCK_SIZE-1) / BLOCK_SIZE;
    dim3 gridDim(num_block_x, num_block_y, 1);
    next_move_kernel<<<gridDim, blockDim>>>(mask_device, rat_count_device, healthy_rat_count_device, exposed_rat_count_device, infectious_rat_count_device, weight_result_device, sum_weight_result_device, neighbor_device, neighbor_start_device, width, height, batch_fraction);
    cudaThreadSynchronize();
    double endTime = CycleTimer::currentSeconds();
