...
class Formatter:
    ...
    def __init__(self, width, height, maxval, fname = "", viz = VizMode.both):
        self.width = width
        self.height = height
        self.digits = int(math.ceil(math.log10(maxval+1)))
        # Pattern that separates lines
        s = "-" * self.digits + "+"
        self.separator = "+" + s * self.width
        self.file = None
        self.stdscr = None
        self.display = None
        vm = VizMode()
        ...
        if vm.doHeatMap(viz):
            self.display = Display(width = self.width, height = self.height, maxval = maxval)
        self.reset()
    ...
    def show(self, populations, period = 0.0):
        ...
        for row in range(self.height):
            rstart = row * self.width
            rvals = populations[rstart:rstart+self.width]
            line = "|"
            for v in rvals:
...
def cstring(c):
    ...
    # fields = [("%.2x" % int(120*x)) for x in c]
    return "#" + "".join(fields)
...
class Display:
    ...
    def __init__(self, width, height,  maxdim = 800, maxval = 100):
        importSpecial()
        self.width = width
        self.height = height
        nodeCount = width * height
        loadFactor = maxval / nodeCount
        self.squareSize = maxdim / max(self.width, self.height)
        self.display = Tkinter.Tk()
        self.display.title('GraphRat Simulation of %d X %d maze (load factor = %d)' % (width, height, loadFactor))
        self.frame = Tkinter.Frame(self.display)
        self.frame.pack(fill=Tkinter.BOTH)
        self.canvas = Tkinter.Canvas(self.frame, width = width * self.squareSize, height = height * self.squareSize)
    ...
    def capture(self, fname):
        img = Image.new('RGB', (self.width * self.squareSize, self.height * self.squareSize), "black")
