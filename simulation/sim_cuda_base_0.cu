...
__global__ void
compute_weight_kernel(double load_factor, double *initial_load_factor, int *rat_count, int *infectious_rat_count, int *neighbor, int *neighbor_start, double* result, int width, int height) {
    ...
    if (x < width && y < height){
        double ilf = neighbor_ilf_kernel(load_factor, initial_load_factor, rat_count, infectious_rat_count, neighbor, neighbor_start, thread_index, INT_MAX); // INT_MAX means always compute all
...
extern "C" void compute_all_weights_cuda(state_t *s){
    ...
    // compute_weight_hub_kernel<<<hubGridDim, hubBlockDim>>>(s->load_factor, initial_load_factor_device, hub_device, nhub, rat_count_device,infectious_rat_count_device, neighbor_device, neighbor_start_device, hub_weight_result_device);
    ...
    compute_weight_kernel<<<gridDim, blockDim>>>(s->load_factor, initial_load_factor_device, rat_count_device, infectious_rat_count_device, neighbor_device, neighbor_start_device, weight_result_device, width, height);
    ...
    // cudaMemcpy(hub_result_host, hub_weight_result_device, sizeof(double) * nhub, cudaMemcpyDeviceToHost);
