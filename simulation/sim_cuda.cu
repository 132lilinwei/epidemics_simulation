...
double *infection_prob_result_device;
...
extern "C" void init_cuda(state_t *s) {
    ...
    alpha = s->alpha;
    beta = s->beta;
    cudaMalloc(&exposed_rat_count_device, sizeof(double) * nnode);
    cudaMalloc(&infection_prob_result_device, sizeof(double) * nnode);
...
extern "C" void clean_cuda() {
    ...
    cudaFree(infection_prob_result_device);
}
...
extern "C" void compute_infection_prob_cuda(state_t *s) {
    graph_t *g = s->g;
    int width = g->width;
    int height = g->height;
    int nnode = g->nnode;
    ...
    int num_block_y = (height+BLOCK_SIZE-1) / BLOCK_SIZE;
    dim3 gridDim(num_block_x, num_block_y, 1);
    compute_infection_prob_kernel<<<gridDim, blockDim>>>(alpha, beta, infectious_rat_count_device, exposed_rat_count_device, rat_count_device, infection_prob_result_device, width, height);
    cudaThreadSynchronize();
    ...
    cudaMemcpy(s->node_prob, infection_prob_result_device, sizeof(double) * nnode, cudaMemcpyDeviceToHost);
